...
<input type="hidden" id="act" name="act"/>
	...
	<div class="sub_sch_box">
		<div class="row">
			<div class="input_box">
				<form:input path="searchKeyword" placeholder="게시판명을 입력하세요."/>
			</div>
	...
	<table class="board_list th_bgc">
		...
		<c:forEach items="${resultList}" var="list" varStatus="status">
			<tr>
				...
				<td>
					<c:choose>
						<c:when test="${list.boardSkin eq 'board'}">일반형</c:when>
						<c:when test="${list.boardSkin eq 'notice'}">공지형</c:when>
						<c:when test="${list.boardSkin eq 'media'}">미디어</c:when>
						<c:when test="${list.boardSkin eq 'faq'}">FAQ</c:when>
						<c:when test="${list.boardSkin eq 'link'}">링크</c:when>
					</c:choose>
				</td>
				<td>
					<c:choose>
						<c:when test="${list.registAuth eq 'E'}">비회원</c:when>
						<c:when test="${list.registAuth eq 'U'}">회원</c:when>
						<c:when test="${list.registAuth eq 'A'}">관리자</c:when>
					</c:choose>
				</td>
				<td>
					<c:choose>
						<c:when test="${list.viewAuth eq 'E'}">비회원</c:when>
						<c:when test="${list.viewAuth eq 'U'}">회원</c:when>
						<c:when test="${list.viewAuth eq 'A'}">관리자</c:when>
					</c:choose>
				</td>
				<td><c:out value="${list.registerId}"/></td>
				<td><c:out value="${list.registerName}"/></td>
				<td><c:out value="${list.regDate}"/></td>
				<td><a href="javascript:void(0);" onClick="fn_edit('regist', '${list.boardCode}');" class="btn-layerClose">복사</a></td>
