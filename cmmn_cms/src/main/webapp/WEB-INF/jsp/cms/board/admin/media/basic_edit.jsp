...
<script type="text/javascript" defer="defer">
	...
	function fn_custom_vali(obj, index) {
		...
		if($(obj).attr('name') == 'fileVali2') {
			if($('#fileTable2').find('tr').not(':hidden').length == 0) {
				return false;
			}
		}
		return true;
				...
				<td>
					<c:choose>
						...
						<c:when test="${list.indexOf('attachFile') != -1}">
							...
							<c:out value="${cFn:kssFileInclude('fileDataList1', boardData.attachCnt, null, 100, 'MB', true, fileList)}" escapeXml="false"/>
<%-- 						<c:out value="${cFn:onFileInclude('fileTable', 'fileDataList1', boardData.attachCnt, 'attach', 50, 'MB', fileList)}" escapeXml="false"/> --%>
							※ 대용량의 영상은 유투브 업로드후 영상을 내용에 링크해야합니다.
						</c:when>
			...
			<tr>
				...
				<td>
					<input type="hidden" id="fileVali2" name="fileVali2" class="vali check custom" vali-text="썸네일을 업로드해주세요."/>
					<c:out value="${cFn:onFileInclude('fileTable2', 'fileDataList2', 1, 'image', 50, 'MB', fileThumbnailList)}" escapeXml="false"/>
				</td>
			</tr>
		</tbody>
	</table>
