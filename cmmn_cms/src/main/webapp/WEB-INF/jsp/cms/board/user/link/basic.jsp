		...
		<div class="board_wrap mobile">
			<ul>
				<li>
					...
					<div class="title">
						<a href="/board/board_view.html">문제해결력과 협업 능력을 키우는 발명영재교육 수업설계</a>
				...
				<tbody>
				  <c:forEach items="${resultList}" var="list" varStatus="status">
					<tr <c:if test="${list.isTop eq 'Y'}">class="red"</c:if>>
						...
						<td class="${boardData.listViewClasses[status2.index]}">
							<c:choose>
								<c:when test="${field eq 'no'}">
									<c:choose>
										<c:when test="${list.isTop eq 'Y'}">공지</c:when>
										<c:otherwise>
											<a href="${list.textField1}" <c:if test="${list.blankYn ne 'N'}">target="_blank"</c:if> style="cursor:pointer"><c:out value="${paginationInfo.totalRecordCount+1 - ((vo.pageIndex-1) * vo.pageUnit + status.count)}"/></a>
										</c:otherwise>
									</c:choose>
								</c:when>
								<c:when test="${field eq 'register'}">
									<a href="${list.textField1}" <c:if test="${list.blankYn ne 'N'}">target="_blank"</c:if> style="cursor:pointer"><c:out value="${list.writer}" escapeXml="false"/></a>
								</c:when>
								<c:when test="${field eq 'title'}">
									<a href="${list.textField1}" <c:if test="${list.blankYn ne 'N'}">target="_blank"</c:if> style="cursor:pointer">
								...
								<c:when test="${field eq 'useAt'}">
									<a href="${list.textField1}" <c:if test="${list.blankYn ne 'N'}">target="_blank"</c:if> style="cursor:pointer"><c:out value="${list.useAt eq 'Y'?'사용' :'미사용'}"/></a>
								</c:when>
								...
								<c:otherwise>
									<a href="${list.textField1}" <c:if test="${list.blankYn ne 'N'}">target="_blank"</c:if> style="cursor:pointer"><c:out value="${list.get(field)}" escapeXml="false"/></a>
								</c:otherwise>
							</c:choose>
						</td>
						</c:if>
