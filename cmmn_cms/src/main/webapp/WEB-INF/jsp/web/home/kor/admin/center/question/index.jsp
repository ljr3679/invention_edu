...
<form:form commandName="commonVO" name="cmmnForm" id="cmmnForm" method="post">
	...
	<div class="sub_sch_box">
		<div class="row">
				...
				<label for="searchValue1">구분</label>
				<div class="slt_box" style="width: 150px;">
					<select id="searchValue2" name="searchValue2" class="join_select1">
						<option value="">전체</option> 
						<option value="A"<c:if test="${vo.searchValue2 eq 'A'}">selected="selected"</c:if>>발명교육포털사이트</option>
						<option value="B"<c:if test="${vo.searchValue2 eq 'B'}">selected="selected"</c:if>>대한민국학생발명전시회</option>
						<option value="C"<c:if test="${vo.searchValue2 eq 'C'}">selected="selected"</c:if>>대한민국 학생창의력 챔피언대회</option>
						<option value="D"<c:if test="${vo.searchValue2 eq 'D'}">selected="selected"</c:if>>YIP(Young Invenfors Program)</option>
					</select>
				</div>
		...
		<tbody>
 			<c:forEach items="${resultList}" var="list" varStatus="status">
				<tr>
					...
					<td>
						<c:choose>
							<c:when test="${list.typeAt eq 'A'}">
								<c:out value="발명교육포털사이트" />
							</c:when>
							<c:when test="${list.typeAt eq 'B'}">
								<c:out value="대한민국학생발명전시회" />
							</c:when>
							<c:when test="${list.typeAt eq 'C'}">
								<c:out value="대한민국 학생창의력 챔피언대회" />
							</c:when>
							<c:when test="${list.typeAt eq 'D'}">
