...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function fn_regist_data2(name,phone) {
				opener.fn_append_data2(name,phone);
				alert("입력되었습니다.");
			}
	...
	<form:form commandName="commonVO" name="cmmnForm" id="cmmnForm" method="post">
		...
		<table class="board_list th_bgc">
			...
			<tbody>
				...
				<tr>
					<td><a href="javascript:void(0);" onclick="fn_regist_data2('${list.name}','${list.phone}');"><c:out value="${paginationInfo2.totalRecordCount+1 - ((vo.pageIndex2-1) * vo.pageUnit2 + status.count)}"/></a></td>
					<td><a href="javascript:void(0);" onclick="fn_regist_data2('${list.name}','${list.phone}');"><c:out value="${list.id}"/></a></td>
					<td><a href="javascript:void(0);" onclick="fn_regist_data2('${list.name}','${list.phone}');"><c:out value="${list.name}"/></a></td>
					<td><a href="javascript:void(0);" onclick="fn_regist_data2('${list.name}','${list.phone}');"><c:out value="${list.birth}"/></a></td>
					<td><a href="javascript:void(0);" onclick="fn_regist_data2('${list.name}','${list.phone}');"><c:out value="${list.email}"/></a></td>
					<td><a href="javascript:void(0);" onclick="fn_regist_data2('${list.name}','${list.phone}');"><c:out value="${list.phone}"/></a></td>
				</tr>
				</c:forEach>
