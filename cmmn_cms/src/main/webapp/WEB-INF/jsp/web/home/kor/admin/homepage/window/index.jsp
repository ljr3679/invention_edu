...
<form:form commandName="commonVO" name="cmmnForm" id="cmmnForm" method="post">
	...
	<div class="sub_sch_box">
		<div class="row">
				<label for="searchValue1">공개여부</label>
				<div class="slt_box" style="width: 10%;">
					<select id="searchValue1" name="searchValue1" class="join_select1">
						<option value="">전체</option>
						<option value="Y"<c:if test="${vo.searchValue1 eq 'Y'}">selected="seleted"</c:if>>공개</option>
						<option value="N"<c:if test="${vo.searchValue1 eq 'N'}">selected="seleted"</c:if>>비공개</option>
					</select>
				</div>
				<label style="margin-left: 20px;" for="searchKeyword">제목</label>
				<div class="input_box">
					<input id="searchKeyword" name="searchKeyword" placeholder="검섹어를 입력하세요." type="text" value="${vo.searchKeyword}"/>
				</div>
		...
		<tbody>
 			<c:forEach items="${resultList}" var="list" varStatus="status">
				<tr>
					...
					<td>
						<c:out value="${list.startDate} ~ ${list.endDate}"/>
					</td>
					<td>
						<c:out value="${list.regDate}"/>
					</td>
					<td><c:out value="${list.horizontalWidth}"/> X <c:out value="${list.verticalWidth}"/></td>
