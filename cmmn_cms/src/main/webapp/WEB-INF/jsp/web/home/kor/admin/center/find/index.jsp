...
<form:form commandName="commonVO" name="cmmnForm" id="cmmnForm" method="post">
	...
	<div class="sub_sch_box">
		<div class="row">
				<label for="searchValue1">지역</label>
				...
				<%-- <label for="searchValue2">사용여부</label>
				<div class="slt_box" style="width: 150px;">
				...
				<div class="slt_box" style="width: 150px;">
					<select id="searchValue3" name="searchValue3" class="join_select1">
						...
						<option value="1"<c:if test="${vo.searchValue3 eq '1'}">selected="selected"</c:if>>발명교육센터명</option>
						<option value="2"<c:if test="${vo.searchValue3 eq '2'}">selected="selected"</c:if>>소속지원청</option>
						<option value="3"<c:if test="${vo.searchValue3 eq '3'}">selected="selected"</c:if>>주소</option>
					</select>
				</div>
				<div class="input_box">
	...
	<table class="board_list th_bgc">
		...
		<thead>
			<tr>
				<th scope="col">No</th>
				<th scope="col">지역</th>
				<th scope="col">발명교육센터명</th>
				<th scope="col">소속지원청</th>
				<th scope="col">주소</th>
		...
		<tbody>
 			<c:forEach items="${resultList}" var="list" varStatus="status">
				<tr>
					...
					<td>
						<c:forEach items="${region}" var="regionVar" varStatus="status">
							<c:if test="${list.regionRef eq regionVar.no}">
								<c:out value="${regionVar.dataName}" />
							</c:if>
						</c:forEach>
						<c:out value="${list.regionName}" />
					</td>
					<td>
						<a onClick="fn_edit('modify', '${list.no}');" style="cursor:pointer"><c:out value="${list.title}"/></a>
					</td>
					<td>
						<c:out value="${list.agency}" />
					</td>
					<td>
						<c:out value="${list.address} ${list.addressDetail}"/>
					</td>
					<td>
						<c:out value="${cFn:decrypt(list.phone)}"/>
					</td>
